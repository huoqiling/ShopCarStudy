...
public class SwipeCardLayout extends RelativeLayout {
    ...
    private void init() {
        ...
        mWidth = 900;
        mHeight = 1200;
        mTranslate = (int) ((mHeight - mHeight * mScale) / 2 + density * 10) - 30;
...
//        mDy = density * 5;
    ...
    public void setAdapter(CardAdapter adapter) {
        ...
        adapter.bindData(adapter.tQueue.poll(), swipeLayout);
        adapter.bindData(adapter.tQueue.poll(), swipeLayout1);
...
//        Log.i(TAG, "onDraw: " + x + " " + y);
    ...
    public void rightOut(final SwipeLayout swipeLayout, final float translationX1) {
        final RelativeLayout parent = (RelativeLayout) getParent();
        int tx = parent.getWidth();
        PropertyValuesHolder holder = PropertyValuesHolder.ofFloat("translationX", translationX1, tx);
        ObjectAnimator animator = ObjectAnimator.ofPropertyValuesHolder(this, holder);
        animator.setDuration(100);
        animator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (adapter.tQueue.size() > 0) {
                    adapter.bindData(adapter.tQueue.poll(), parent.getChildAt(0));
                } else {
                    parent.removeView(swipeLayout);
                    return;
                }
